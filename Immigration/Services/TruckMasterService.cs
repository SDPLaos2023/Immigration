...
namespace Immigration.Services
{
    public class TruckMasterService
    {
        ...
        public string TruckMasterInsert(TruckMasterModel _TruckMaster)
        {
            try
            {
                var command = _context.Database.GetDbConnection().CreateCommand();
                command.CommandText = "SP_TRUCK_MASTER_DATA_INSERT";
        ...
        public DataTable TruckMasterSelect(string TruckLicencePlateNo)
        {
            try
            {
                var command = _context.Database.GetDbConnection().CreateCommand();
